# Public Link One-Time Consumption Lifecycle Package-Owned Surface

Batch: `public-link-one-time-consumption-lifecycle-package-owned-surface-slice`
...
- no real one-time token consumption
- no persistent consumed_at write
- no token storage or token generation runtime
- no public file download
- no file content response
